...
假设一组 P 个向量对![$$ \left({x}_1,{y}_1\right),\left({x}_2,{y}_2\right),\cdots, \left({x}_P,{y}_P\right) $$](A458833_1_En_5_Chapter_IEq1.gif)，它们是函数映射![$$ y=\phi (x):x\in {R}^N,y\in {R}^M $$](A458833_1_En_5_Chapter_IEq2.gif)的例子。

三层网络中的信息处理等式如下所示。一个输入向量如下:

![$$ ne{t}_{pj}^h=\sum \limits_{i=1}^N{\omega}_{ji}^h{x}_{pi}+{\theta}_j^h $$](A458833_1_En_5_Chapter_Equa.gif)

一个输出节点如下:

![$$ {i}_{pj}={f}_j^h\left( ne{t}_{pj}^h\right) $$](A458833_1_En_5_Chapter_Equb.gif)

输出节点的方程式如下:

![$$ {\displaystyle \begin{array}{l} ne{t}_{pk}^o=\sum \limits_{j=1}^L{\omega}_{kj}^o{i}_{pj}+{\theta}_k^o\\ {}\ {o}_{pk}={f}_k^o\left( ne{t}_{pk}^o\right)\end{array}} $$](A458833_1_En_5_Chapter_Equc.gif)

### 输出层权重的更新

下列等式是误差项:

![$$ {\displaystyle \begin{array}{l}{E}_p=\frac{1}{2}\sum \limits_k{\left({y}_{pk}-{o}_{pk}\right)}^2\\ {}\frac{\partial {E}_p}{\partial {\omega}_{kj}^o}=-\left({y}_{pk}-{o}_{pk}\right)\frac{\partial {f}_k^o}{\partial \left( ne{t}_{pk}^o\right)}\frac{\partial \left( ne{t}_{pk}^o\right)}{\partial {\omega}_{kj}^o}\end{array}} $$](A458833_1_En_5_Chapter_Equd.gif)

输出层权重方程中的最后一个因子如下:

![$$ \frac{\partial \left( ne{t}_{pk}^o\right)}{\partial {\omega}_{kj}^o}=\left(\frac{\partial }{\partial {\omega}_{kj}^o}\sum \limits_{j=1}^L{\omega}_{kj}^o{i}_{pj}+{\theta}_k^o\right)={i}_{pj} $$](A458833_1_En_5_Chapter_Eque.gif)

负渐变如下:

![$$ -\frac{\partial {E}_p}{\partial {\omega}_{kj}^o}=\left({y}_{pk}-{o}_{pk}\right){f}_k^{o^{\prime }}\left( ne{t}_{pk}^o\right){i}_{pj} $$](A458833_1_En_5_Chapter_Equf.gif)

输出层上的权重根据以下内容更新:

![$$ {\displaystyle \begin{array}{l}{\omega}_{kj}^{\mathrm{o}}\left(t+1\right)={\omega}_{kj}^o(t)+{\varDelta}_p{\omega}_{kj}^o(t)\\ {}{\varDelta}_p{\omega}_{kj}^o=\eta \left({y}_{pk}-{o}_{pk}\right){f}_k^{o^{\prime }}\left( ne{t}_{pk}^o\right){i}_{pj}\end{array}} $$](A458833_1_En_5_Chapter_Equg.gif)

输出函数有两种形式。

*   ![$$ {f}_k^o\left( ne{t}_{jk}^o\right)= ne{t}_{jk}^o $$](A458833_1_En_5_Chapter_Equh.gif)

*   ![$$ {f}_k^o\left( ne{t}_{jk}^o\right)={\left(1+{e}^{- ne{t}_{jk}^o}\right)}^{-1} $$](A458833_1_En_5_Chapter_Equi.gif)

### 隐藏层权重的更新

仔细观察前面所示的由一层隐藏神经元和一个输出神经元组成的网络。当输入向量在网络中循环时，对于当前的一组权重，有一个输出预测。自动地，总误差在某种程度上与隐藏层中的输出值相关。方程式如下:

![$$ {\displaystyle \begin{array}{l}{E}_p=\frac{1}{2}\sum \limits_k{\left({y}_{pk}-{o}_{pk}\right)}^2\\ {}=\frac{1}{2}\sum \limits_k{\left({y}_{pk}-{f}_k^o\left( ne{t}_{pk}^o\right)\right)}^2\\ {}=\frac{1}{2}\sum \limits_k{\left({y}_{pk}-{f}_k^o\left(\sum \limits_j{\omega}_{kj}^o{i}_{pj}+{\theta}_k^o\right)\right)}^2\end{array}} $$](A458833_1_En_5_Chapter_Equj.gif)

你可以利用这个事实来计算 E <sub>p</sub> 相对于隐藏层权重的梯度。【T2![$$ {\displaystyle \begin{array}{l}\frac{\partial {E}_p}{\partial {\omega}_{ji}^h}=\frac{1}{2}\sum \limits_k\frac{\partial }{\partial {\omega}_{ji}^h}{\left({y}_{pk}-{o}_{pk}\right)}^2\\ {}\kern1em =-\sum \limits_k\left({y}_{pk}-{o}_{pk}\right)\frac{\partial {o}_{pk}}{\partial \left( ne{t}_{pk}^o\right)}\frac{\partial \left( ne{t}_{pk}^o\right)}{\partial {i}_{pj}}\frac{\partial {i}_{pj}}{\partial \left( ne{t}_{pj}^h\right)}\frac{\partial \left( ne{t}_{pj}^h\right)}{\partial {\omega}_{ji}^h}\end{array}} $$](A458833_1_En_5_Chapter_Equk.gif)

等式中的每个因子都可以从前面的等式中显式计算出来。结果如下:

![$$ \frac{\partial {E}_p}{\partial {\omega}_{ji}^h}=-\sum \limits_k\left({y}_{pk}-{o}_{pk}\right){f}_k^{o\hbox{'}}\left( ne{t}_{pk}^o\right){\omega}_{kj}^o{f}_j^{h\hbox{'}}\left( ne{t}_{pj}^h\right){x}_{pi} $$](A458833_1_En_5_Chapter_Equl.gif)

### BPN 摘要

将输入向量![$$ {X}_p={\left({x}_{p1},{x}_{p2},\dots, {x}_{pN}\right)}^t $$](A458833_1_En_5_Chapter_IEq3.gif)应用到输入单元。

计算隐藏层单元的净输入值。

![$$ ne{t}_{pj}^h=\sum \limits_{i=1}^N{\omega}_{ji}^h{x}_{pi}+{\theta}_j^h $$](A458833_1_En_5_Chapter_Equm.gif)T2】

计算隐藏层的输出。

![$$ {i}_{pj}={f}_j^h\left( ne{t}_{pj}^h\right) $$](A458833_1_En_5_Chapter_Equn.gif)T2】

计算每个单位的净输入值。

![$$ ne{t}_{pk}^o=\sum \limits_{j=1}^L{\omega}_{kj}^o{i}_{pj}+{\theta}_k^o $$](A458833_1_En_5_Chapter_Equo.gif)T2】

计算输出。

![$$ {o}_{pk}={f}_k^o\left( ne{t}_{pk}^o\right) $$](A458833_1_En_5_Chapter_Equp.gif)T2】

计算输出单位的误差项。

![$$ {\delta}_{pk}^o=\left({y}_{pk}-{o}_{pk}\right){f}_k^{o\hbox{'}}\left( ne{t}_{pk}^o\right) $$](A458833_1_En_5_Chapter_Equq.gif)T2】

计算隐藏单元的误差项。

![$$ {\delta}_{pj}^h={f}_j^{h\hbox{'}}\left( ne{t}_{pj}^h\right)\sum \limits_k{\delta}_{pk}^o\ {\omega}_{kj}^o $$](A458833_1_En_5_Chapter_Equr.gif)T2】

更新输出图层的权重。

![$$ {\omega}_{kj}^o\left(t+1\right)={\omega}_{kj}^o(t)+\eta {\delta}_{pk}^o{i}_{pj} $$](A458833_1_En_5_Chapter_Equs.gif)T2】

更新隐藏层的权重。

![$$ {\omega}_{ji}^h\left(t+1\right)={\omega}_{ji}^h(t)+\eta {\delta}_{pj}^h{x}_i $$](A458833_1_En_5_Chapter_Equt.gif)T2】
...
![$$ {w}_{ij}\left(t+1\right)={w}_{ij}(t)+\eta {y}_j(t){x}_i(t) $$](A458833_1_En_5_Chapter_Equu.gif)T2】
