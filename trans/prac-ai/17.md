...
执行清单 [17-12](#Par121) 中的代码后得到的结果将是贯穿本章开发的 Windows 窗体应用的一个实例(图 [17-6](#Fig6) )。奖励函数包含目标状态的奖励为 1，任何其他状态的奖励为-0.01。一旦代理完成第一集，目标状态(退出)将包含每个动作的奖励 1；即`Q('Exit', {up, down, left, right}) = 1`。
